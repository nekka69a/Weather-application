...
export const apiCallLocationAndAirQuality = function () {
  document
    .querySelector("form")
    .addEventListener("submit", async function (evt) {
      ...
      try {
        ...
        document.querySelector(
          ".icon"
        ).innerHTML = `<img class="weather-icon" src="https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png" alt="" />`;
        document.querySelector(".weather-city").innerHTML =
          data.weather[0].description;
        document.querySelector(
          ".humidity-icon"
        ).innerHTML = `<img class="img-icon" src="../assets/humidite (1).png "alt="" />`;
        document.querySelector(
          ".humidity-data"
        ).innerHTML = `Humidité : ${data.main.humidity} %`;
        document.querySelector(
          ".minmax-icon"
        ).innerHTML = `<img class="img-icon" src="../assets/thermometre.png "alt="" />`;
        document.querySelector(".minmax-data").innerHTML = `Min: ${Math.round(
          data.main.temp_min
        )}°C</br>Max: ${Math.round(data.main.temp_max)}°C`;
        document.querySelector(
          ".wind-icon"
        ).innerHTML = `<img class="img-icon" src="../assets/vent (1).png "alt="" />`;
        document.querySelector(
          ".wind-speed"
        ).innerHTML = `${data.wind.speed} m/s</br>Direction: ${data.wind.deg}°`;
        document.querySelector(
          ".pressure-icon"
        ).innerHTML = `<img class="img-icon" src="../assets/pression.png "alt="" />`;
        document.querySelector(
          ".pressure-data"
        ).innerHTML = `${data.main.pressure} hPa`;
        document.querySelector(
          ".visibility-icon"
        ).innerHTML = `<img class="img-icon" src="../assets/oeil.png "alt="" />`;
...
const apiCallAir = async function (ville) {
  try {
    // Appel api pour la géolocalisation
    let urlLocation = `http://api.openweathermap.org/geo/1.0/direct?q=${ville}&appid=${apikey}`;
    ...
    let lat = dataLocation[0].lat;
    let lon = dataLocation[0].lon;

    // Second appel api pour la qualité de l'air
    let urlAir = `http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${apikey}`;
    ...
    document.querySelector(
      ".indice"
    ).innerHTML = `Indice de qualité de l'air : ${dataAir.list[0].main.aqi}`;
    document.querySelector(
      ".co"
    ).innerHTML = `CO : ${dataAir.list[0].components.co} µg/m³`;
    document.querySelector(
      ".no"
    ).innerHTML = `NO : ${dataAir.list[0].components.no} µg/m³`;
    document.querySelector(
      ".no2"
    ).innerHTML = `NO² : ${dataAir.list[0].components.no2} µg/m³`;
    document.querySelector(
      ".o3"
    ).innerHTML = `O3 : ${dataAir.list[0].components.o3} µg/m³`;
    document.querySelector(
      ".so2"
    ).innerHTML = `SO² : ${dataAir.list[0].components.so2} µg/m³`;
    document.querySelector(
      ".pm2"
    ).innerHTML = `PM2.5 : ${dataAir.list[0].components.pm2_5} µg/m³`;
    document.querySelector(
      ".pm10"
    ).innerHTML = `PM10 : ${dataAir.list[0].components.pm10} µg/m³`;
    document.querySelector(
      ".nh3"
    ).innerHTML = `NH3 : ${dataAir.list[0].components.nh3} µg/m³`;
  } catch (err) {
    console.error("Erreur : " + err);
  }
};
